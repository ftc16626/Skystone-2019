...
@TeleOp(name="Mecanum Test 3", group="Mecanum")
public class MecanumTest3 extends OpMode {
  ...
  @Override
  public void loop() {
    double theta = Math.atan(gamepad1.left_stick_x + gamepad1.left_stick_y) + Math.PI / 4;
    double magnitude = Math.hypot(gamepad1.left_stick_x, gamepad1.left_stick_y);
    double rot = gamepad1.right_stick_x;
    magnitude = magnitude * (100 / 127) - rot;
    double newX = Math.cos(theta) * magnitude;
    double newY = Math.cos(theta) * magnitude;
